...
public class MainActivity extends BaseActivity {
	...
	private void requestShowList(String showTypeId) {
		DiscoverListObserver discoverListObserver = new DiscoverListObserver() {
			@Override
			public void getJsonData(int type, int refreshType,
					List<TicketModel> data, String erro) {
				if (data != null) {
					if (data.size() > 0) {
		...
		jsonHelper.sendJsonManager.helpShowListJsonData(
				JsonHttpConstant.JSONDATA_TYPE_SHOW_LIST, "852", showTypeId,
				venueId + "", timeId + "", 100, showListPage, true,
				discoverListObserver, 0);
	}
	...
	class ShowListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				showViewHolder.tv_item_price = (TextView) convertView
						.findViewById(R.id.tv_item_price);
				showViewHolder.tv_item_title = (TextView) convertView
						.findViewById(R.id.tv_item_title);
				showViewHolder.tv_item_num = (TextView) convertView
						.findViewById(R.id.tv_item_num);
				showViewHolder.iv_item_poster = (ImageView) convertView
						.findViewById(R.id.iv_item_poster);
			...
			showViewHolder.tv_item_price.setText(ticketModel.getLow_price()
					+ " - " + ticketModel.getHighest_price());
			showViewHolder.tv_item_num.setText("" + ticketModel.getFreecount()
					+ "张");
			showViewHolder.iv_item_poster
					.setImageResource(R.drawable.discover_default);
